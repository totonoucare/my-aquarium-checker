...
async function searchOne(recipeId, mode, env, request) {
  ...
  if (!recipe) {
    return { recipeId, ok: false, error: 'unknown_recipe' };
  }
  ...
  const cacheKey = new Request(`${new URL(request.url).origin}/api/rakuten-products?recipeId=${recipeId}&mode=${mode}`, request);
...
export async function onRequestGet(context) {
  ...
  const recipeIds = (url.searchParams.get('recipeIds') || url.searchParams.get('recipeId') || '')
    .split(',').map((s) => s.trim()).filter(Boolean).slice(0, 14);
  ...
  const results = await Promise.all(recipeIds.map((id) => searchOne(id, mode, env, request)));
