...
function renderGearForFish(fish) {
  const baseCare = [
    ["水温計・カルキ抜き", "どの魚でも最初に用意しておきたい基本用品。", "/comparison/#care"]
  ];

  let items;
  if (fish.id === "betta") {
    items = [
      ["ベタ用水槽セット", "単独飼育で始めやすいセット。フタつきだと安心。", "/comparison/#starter"],
      ["ヒーター", "ベタは保温が必要になりやすいため、対応水量を確認。", "/comparison/#heater"],
      ["水流が強すぎないフィルター", "ベタは強い水流が苦手なことがあります。", "/comparison/#filter"],
      ["隠れ家・レイアウト", "ベタが落ち着ける隠れ場所や見た目づくりに。", "/comparison/#layout"]
    ].concat(baseCare);
  } else if (fish.id === "angelfish") {
    items = [
      ["60cm水槽セット", "成長後のサイズを考えると60cm以上から検討したい魚。", "/comparison/#starter"],
      ["40〜60cm向けフィルター", "水量と魚の大きさに合わせてろ過に余裕を。", "/comparison/#filter"],
      ["60cm向けLED", "大きめ水槽をきれいに見せたい場合に。", "/comparison/#light"],
      ["熱帯魚の餌", "口のサイズや混泳相手に合わせて選びます。", "/comparison/#food"]
    ].concat(baseCare);
  } else if (fish.id === "corydoras") {
    items = [
      ["45cm水槽セット", "底ものを入れるなら底面スペースに余裕を持ちたいです。", "/comparison/#starter"],
      ["コリドラス用タブレット", "底に沈む餌があると食べさせやすいです。", "/comparison/#food"],
      ["水換え・底砂掃除用品", "底砂に汚れがたまりやすいので掃除道具が便利。", "/comparison/#care"],
      ["フィルター", "底ものを入れる水槽でも水質安定のために用意。", "/comparison/#filter"]
    ].concat(baseCare);
  } else if (["minami", "redcherry", "yamato"].includes(fish.id)) {
    items = [
      ["30cm水槽セット", "エビ水槽を小さく始めたい人向け。", "/comparison/#starter"],
      ["水作系フィルター", "稚エビの吸い込み対策を意識したい場合に。", "/comparison/#filter"],
      ["LEDライト", "水草や隠れ家を楽しむならライトがあると便利。", "/comparison/#light"],
      ["隠れ家・レイアウト", "エビや小型魚の隠れ場所づくりに。", "/comparison/#layout"],
      ["底もの・エビ向け餌", "エビに届きやすい沈む餌が使いやすいです。", "/comparison/#food"]
    ].concat(baseCare);
  } else if (["medaka", "whitecloud"].includes(fish.id)) {
    items = [
      ["30cm水槽セット", "ヒーターなし構成でも始めやすいサイズ。", "/comparison/#starter"],
      ["フィルター", "シンプル飼育でも水を安定させやすくなります。", "/comparison/#filter"],
      ["掃除用品", "水換えを楽にできると続けやすいです。", "/comparison/#care"]
    ].concat(baseCare);
  } else {
    items = [
      ["30cm〜45cm水槽セット", "小型熱帯魚なら、魚数に合わせて30〜45cmを検討。", "/comparison/#starter"],
      ["外掛けフィルター", "初心者でも扱いやすい定番のろ過用品。", "/comparison/#filter"],
      ["ヒーター", "熱帯魚は保温が必要になりやすいです。", "/comparison/#heater"],
      ["熱帯魚の餌", "魚の口のサイズに合う餌を選びます。", "/comparison/#food"]
    ].concat(baseCare);
  }
